...
ES_t TimerFPWM_enuCalc_OCR(f32 f32duty,Timer_DutyType value)
{
	ES_t LocalErrorstate =ES_OK;
	  ...
	  {
		  case ON:
			 OCR0  = (uint8_t)(f32duty*counts_Timer);
		     break;
		  case OFF:
			 OCR0 = (uint8_t)(((f32duty*counts_Timer) - counts_Timer)/-1);
			 break;
...
ES_t TimerPhaseCorrect_enuCalc_OCR(f32 f32duty,Timer_DutyType value)
{
	ES_t LocalErrorstate = ES_OK;
	switch(value)
	{
		case ON:
		OCR0  = (uint8_t)((counts_Timer_phasecorrect*f32duty)/2);
		break;
		case OFF:
		OCR0  = (uint8_t)(counts_Timer_phasecorrect/2) - ((counts_Timer_phasecorrect*f32duty)/2);
		break;
